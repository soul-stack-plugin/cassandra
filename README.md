...
Manages a live Apache Cassandra cluster.

A Soul Stack **SoulModule** plugin, built as `soul-mod-cassandra` and launched by the host over
...
The objects and actions it serves are declared in `schema.json`, which is GENERATED from the Go value — `make check` fails if the two disagree.

## Build and check

```sh
make check      # fmt, vet, no-replace, tests, and the schema/manifest gate
make build      # dist/soul-mod-cassandra
```
...
It lived in `soul-stack/examples/module/soul-mod-cassandra` until NIM-825. [ADR-011][adr11]
